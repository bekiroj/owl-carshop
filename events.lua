...
addEventHandler("carshop.buy", root, function(libID, vehID, vehName, price)
	if source and tonumber(libID) and tonumber(vehID) then
		if exports.slua_global:takeMoney(source, price) then
			...
			local var1, var2 = exports['slua_vehicle']:getRandomVariant(vehID)
			...
			call( getResourceFromName("slua_items" ), "deleteAll", 3, smallestID)
			exports.slua_global:giveItem(source, 3, smallestID)
			destroyElement(veh)
			exports['slua_vehicle']:reloadVehicle(smallestID)

			source:outputChat('['..exports.config:getServerName()..']#D0D0D0 Başarıyla '..vehName..' markalı aracı satın aldın!',195,184,116,true)
		else
			source:outputChat('['..exports.config:getServerName()..']#D0D0D0 Bu aracı satın almak için paranız yeterli değil. Gerekli miktar: '..exports.slua_global:formatMoney(price)..''..exports.config:getServerMoneyType(),195,184,116,true)
		end
	end
end)